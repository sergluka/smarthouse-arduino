...
void signerPresentation(MyMessage &msg, uint8_t destination) {
	...
	if (destination == GATEWAY_ADDRESS) {
		SIGN_DEBUG(PSTR("Waiting for GW to send_message signing preferences...\n"));
	...
	if (mGetCommand(msg) == C_INTERNAL) {
		...
		if (msg.type == I_NONCE_REQUEST) {
			...
			if (msg.data[1] & SIGNING_PRESENTATION_REQUIRE_WHITELISTING) {
				...
				// We received an indicator that the sender does not require us to sign all messages we send_message to it
	...
	if (DO_SIGN(msg.destination) && msg.sender == _nc.nodeId) {
		if (skipSign(msg)) {
			...
			if (_signingNonceStatus == SIGN_OK) {
				...
				SIGN_DEBUG(PSTR("Message to send_message could not be signed!\n"));
				return false;
			}
