...
bool transportRouteMessage(MyMessage &message) {
	...
	if (_transportSM.findingParentNode && destination != BROADCAST_ADDRESS) {
		debug(PSTR("!TSP:FPAR:ACTIVE (msg not send_message)\n"));
		// request to send_message a non-BC message while finding parent active, abort
		return false;
	}
		...
		route = BROADCAST_ADDRESS;		// message to BC does not require routing
			...
			if (route == AUTO) {
				...
				if (message.last != _nc.parentNodeId) {
					...
					// route unknown and msg received from parent, send_message it to destination assuming in rx radius
					route = destination;
				}
			}
		#else
